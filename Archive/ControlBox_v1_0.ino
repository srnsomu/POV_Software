...
void loop()
{
  ...
  /* Put all the user input data into one unsigned int and send it over to the MAX32.
     There is one byte of unused space (the most significant byte) that can be used
     for parity/checksums and the like. */
  combinedData = mstat + (mx<<8) + (my<<16);
  mySerial.write(combinedData);
  numPacketsSent++;
