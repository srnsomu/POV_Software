...
#define CURSOR_DELAY 1000 //How many ms between toggling the cursor flash
...
void loop()
{
  ...
  if(Serial1.available() > 3) {
    ...
    extraByte = Serial1.read(); //Extra byte, currently unused but could be used for checksum etc.  
  ...
  if((millis() - lastActTime) % CURSOR_DELAY == 0) {
    //cursorFlag = ~cursorFlag; //Toggles cursor on/off - no cursor in prototype test
  }
